...
# Based on: Peter Jung ptr1337 <admin@ptr1337.dev>
# Based on: Piotr Gorski <piotrgorski@cachyos.org>
...
# - "generic_v3" (optimized for Intel 10th gen+ / AMD Zen2+, recommended for i7-10700K)
...
# create_llvm_prof --binary=/usr/src/debug/linux-kiro/vmlinux --profile=propeller.data --format=propeller --propeller_output_module_name --out=propeller_cc_profile.txt --propeller_symorder=propeller_ld_profile.txt
...
_package() {
    ...
    if _is_lto_kernel; then
        provides+=(linux-kiro-lto=$_kernver)
        replaces=(linux-kiro-lto)
    fi
...
_package-headers() {
    ...
    if _is_lto_kernel; then
        provides+=(linux-kiro-lto-headers=$_kernver)
        replaces=(linux-kiro-lto-headers)
